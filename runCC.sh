...
#$HOME/projects/def-mlorincz/scripts/MasterDAT/MasterDAT.sh -t $THREADS -m $MEM_THREAD -b -g $GENOME # BAM file input - will find ALL bam files in subdirectories of current location (one level down)

#$HOME/projects/def-mlorincz/scripts/MasterDAT/MasterDAT.sh -t $THREADS -m $MEM_THREAD -i $SRA_FILE -g $GENOME -F # Only produce fastq files after downloading